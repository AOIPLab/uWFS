...
#reference_coordinates_filename = reference_directory + 'reference_initial.txt' # JDR- use this instead of line above to reference the theoretical centreoid locations based on geometry instead of measuremed reference wavefront_colormap
...
lenslet_focal_length_m = 5.2e-3 #14.6e-3
pixel_size_m = 5.86e-6 #(5.04e-6)*500/350 #5.04e-6
...
sensor_remove_tip_tilt = False #True
...
mirror_command_max = .7 #.36 #1.0
mirror_command_min = -.7 #-.36 #-1.0
mirror_settling_time_s = 0.001
mirror_n_actuators = 97

poke_command_max = .2 #0.3
poke_command_min = -.2 #-0.3
poke_n_command_steps = 5
poke_invert_on_demand = False

ctrl_dictionary_max_size = 10

loop_n_control_modes = 94 #94 50
loop_gain = -.12 #-.12 #-0.15
loop_loss = .15 #.15 #0.14
loop_update_rate = 50.0 #500.0
...
#zernike_plot_print_func = lambda val: '%0.3f um defocus'%(val*1e6)
#zernike_plot_print_func = lambda val: '%0.3f defocus [m]'%(1.277*2/(val*1e6)) #JDR to do radius in m
zernike_plot_print_func = lambda val: '%0.4f diopters defocus'%(val*1e6/(1.277*2)) #JDR to do diopters
...
wavefront_contrast_limits = (-5e-6,5e-6)
